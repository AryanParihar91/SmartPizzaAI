...
public class AdminServiceImpl implements AdminService {
	...
	public AdminAnalyticsResponse getAnalytics() {
		...
		for (CustomerOrder order : orders) {
			...
			if (order.getOrderItems() != null) {
				...
				for (OrderItem item : order.getOrderItems()) {
					...
					categoryRevenueMap.put(categoryName,
							categoryRevenueMap.getOrDefault(categoryName, 0.0) + itemRevenueWithGst);
				}
			}
		}

		List<TopSellingPizzaResponse> topSellingPizzas = new ArrayList<>(pizzaSalesMap.values());
		...
		List<CategoryRevenueResponse> categoryRevenueResponses = new ArrayList<>();
		...
		for (Map.Entry<String, Double> entry : categoryRevenueMap.entrySet()) {
			...
			categoryRevenueResponses.add(response);
		}

		categoryRevenueResponses.sort(Comparator.comparing(CategoryRevenueResponse::getRevenue).reversed());

		long nonCancelledOrders = orders.stream().filter(order -> order.getOrderStatus() != OrderStatus.CANCELLED)
				.count();
		...
		analytics.setFinalRevenueWithGst(finalRevenueWithGst);
		analytics.setAverageOrderValue(averageOrderValue);
		analytics.setTopSellingPizzas(topSellingPizzas);
		analytics.setCategoryRevenue(categoryRevenueResponses);
